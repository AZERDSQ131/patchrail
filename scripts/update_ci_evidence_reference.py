...
DOCS_BLOCK_RE = re.compile(
    ...
    r"  package-smoke, and the open-source evidence snapshot job\. The uploaded\n"
    r"  `patchrail-open-source-evidence` artifact includes the general snapshot, Agent\n"
...
def update_ci_reference(root: Path, run_url: str, commit: str, *, dry_run: bool) -> list[Path]:
    ...
    docs_path = root / "docs" / "openai-open-source-evidence.md"
