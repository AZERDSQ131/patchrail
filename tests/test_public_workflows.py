...
def test_evidence_snapshot_summarizes_public_open_source_signals_without_write_actions() -> None:
    ...
    assert payload["signals"]["ci_fixtures"] == 188
    assert payload["signals"]["ci_expected_files"] == 188
    assert payload["signals"]["ci_benchmark_passed"] == 188
...
def test_roadmap_audit_tracks_versions_and_weeks_without_external_claims() -> None:
    ...
    assert payload["versions"]["v0.2.0"]["signals"]["ci_fixtures"] == 188
