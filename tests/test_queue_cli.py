...
class PatchRailQueueTests(unittest.TestCase):
            ...
            self.assertEqual(
                run_patchrail(
                    [
                        ...
                        risky_item["id"],
                        "--reason",
                        "revenue-safety policy: local-only human approval required",
                    ]
                ).returncode,
                0,
            )
    ...
    def test_queue_policy_resolve_skips_blocked_items_and_rejects_blocked_proposals(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                resolved["reason"], "revenue-safety policy: local-only human approval required"
            )
